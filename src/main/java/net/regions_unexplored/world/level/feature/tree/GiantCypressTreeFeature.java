...
public class GiantCypressTreeFeature extends Feature<RuTreeConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<RuTreeConfiguration> context) {
        ...
        while (placeCheck <= height_main) {
            if (isReplaceableDirt(level, placePos) || level.getBlockState(placePos).getMaterial().isReplaceable()||level.getBlockState(placePos).is(RuTags.BRANCHES)||level.getBlockState(placePos).is(BlockTags.LEAVES)) {
