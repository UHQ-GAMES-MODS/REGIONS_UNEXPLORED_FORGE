package net.regions_unexplored.data.block;

import net.minecraft.world.level.block.state.properties.WoodType;
import net.regions_unexplored.RegionsUnexploredMod;

public class RuWoodTypes {
    public static WoodType BAOBAB = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":baobab", RuBlockSetType.BAOBAB));
    public static WoodType BLACKWOOD = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":blackwood", RuBlockSetType.BLACKWOOD));
    public static WoodType BLUE_BIOSHROOM = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":blue_bioshroom", RuBlockSetType.BLUE_BIOSHROOM));
    public static WoodType BRIMWOOD = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":brimwood", RuBlockSetType.BRIMWOOD));
    public static WoodType COBALT = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":cobalt", RuBlockSetType.COBALT));
    public static WoodType CYPRESS = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":cypress", RuBlockSetType.CYPRESS));
    public static WoodType DEAD = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":dead", RuBlockSetType.DEAD));
    public static WoodType EUCALYPTUS = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":eucalyptus", RuBlockSetType.EUCALYPTUS));
    public static WoodType GREEN_BIOSHROOM = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":green_bioshroom", RuBlockSetType.GREEN_BIOSHROOM));
    public static WoodType JOSHUA = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":joshua", RuBlockSetType.JOSHUA));
    public static WoodType LARCH = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":larch", RuBlockSetType.LARCH));
    public static WoodType MAGNOLIA = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":magnolia", RuBlockSetType.MAGNOLIA));
    public static WoodType MAPLE = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":maple", RuBlockSetType.MAPLE));
    public static WoodType MAUVE = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":mauve", RuBlockSetType.MAUVE));
    public static WoodType PALM = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":palm", RuBlockSetType.PALM));
    public static WoodType PINE = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":pine", RuBlockSetType.PINE));
    public static WoodType PINK_BIOSHROOM = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":pink_bioshroom", RuBlockSetType.PINK_BIOSHROOM));
    public static WoodType REDWOOD = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":redwood", RuBlockSetType.REDWOOD));
    public static WoodType SOCOTRA = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":socotra", RuBlockSetType.SOCOTRA));
    public static WoodType WILLOW = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":willow", RuBlockSetType.WILLOW));
    public static WoodType YELLOW_BIOSHROOM = WoodType.register(new WoodType(RegionsUnexploredMod.MOD_ID + ":yellow_bioshroom", RuBlockSetType.YELLOW_BIOSHROOM));
}
